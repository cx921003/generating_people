...
./run.py sample experiments/config/CSM_pretrained --out_fp tmp --n_samples ${npeople}
echo Done.
...
# added for visualize bodysegments
mkdir -p ${out_fp}/images
for sample_idx in $(seq 0 $((${npeople}-1))); do
    if [  -f tmp/images/${sample_idx}_conditioning.png ]; then
        fullid=$(printf "%04d" ${sample_idx})
        cp tmp/images/${sample_idx}_conditioning.png ${out_fp}/images/${fullid}_sample_conditioning.png
    fi
done
echo Done.

echo Creating images...
./run.py test experiments/config/PM_pretrained --override_dset_suffix tmp --out_fp ${out_fp}
echo Done.
...
for sample_idx in $(seq 0 $((${npeople}-1))); do
    fullid=$(printf "%04d" ${sample_idx})
    # Simulate full dataset.
    cp ${data_fp}/${fullid}_image:png.png ${out_fp}/images/${fullid}_sample_image.png
    cp ${data_fp}/${fullid}_label_vis:png.png ${out_fp}/images/${fullid}_sample_label.png
done
echo Done.
